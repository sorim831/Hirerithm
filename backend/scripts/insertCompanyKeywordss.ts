// backend/scripts/insertCompanyKeywords.ts

import fs from "fs";
import path from "path";
import { db } from "../config/db"; // ✅ 그대로 유지된 db.ts의 export 방식
import CompanyKeyword from "../models/CompanyKeyword"; // 모델 import
...
async function insertKeywordsFromFiles(): Promise<void> {
  // ✅ db.ts는 connect()가 아니라 db() 자체가 연결 함수이므로 이렇게 호출
  await db();
  ...
  // mongoose.disconnect() 직접 불러도 됩니다
